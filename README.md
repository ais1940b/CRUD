# This Project is Based on John Papa Stylle using ngbp boilerplate
...
`encora` is designed to make life easy by providing a basic framework
...
encora also comes prepackaged with the most popular design frameworks
...
The principal goal of `encora` is to set projects up for long-term
success.  So `encora` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `encora`.

But `encora` is not an example of an AngularJS app: this is a
...
getting `encora` up and running. Though `encora` is really simple
...
`encora` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `encora`, like Twitter Bootstrap
...
Technically, `encora` is now ready to go.
...
`encora`. What follows in this section is a quick introduction to the
...
`encora` also includes [Live Reload](http://livereload.com/), so you no
...
default with `encora`) you need to tell Live Reload to allow it. Go to
